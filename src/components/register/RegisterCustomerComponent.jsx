...
import imageAnalisis from '../../assets/18953891_6057221.svg';
...
export const RegisterCustomerComponent = ({ isRoles, isLoading }) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const { loading, error, data, authRegisterCustomer } = useRegisterCustomer();
  const onSubmit = handleSubmit((data) => {
    authRegisterCustomer(data);
    console.log(data);
  });
      ...
      <div>
        ...
        >
          ...
          <div className="flex flex-col">
            ...
            {errors.rol && (
              <span className="text-sm text-red-500">{errors.rol.message}</span>
            )}
            <label
              htmlFor="password"
              className="font-light mt-5 text-2xl text-neutral-900"
